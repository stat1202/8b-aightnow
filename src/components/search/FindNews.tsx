...
export default function FindNews({
  ...
  const stockNames =
    filteredStocks &&
    filteredStocks.map((stock) => stock.stock_name.toLowerCase());
      ...
      (news: News) =>
        ...
        stockNames.some(
          (stockName) =>
            ...
            news.title_en.toLowerCase().includes(stockName) ||
            news.content_en.toLowerCase().includes(stockName),
        ),
    );
  console.log(filteredNews, 'news');
  // 현재 표시되는 뉴스 항목 슬라이싱
